  ...
  substitutions:
    params:
      vars:
        ...
        thing: pets
        Question: a) Would you expect the distance to the nearest hardware store,
          i.e. number of miles between the nearest hardware store and home to be symmetric,
          right skewed, or left skewed?
      part1:
        ans1:
          value: 'Right skewed: The distribution of number of distance to the nearest
            hardware store is likely right skewed as there is a natural boundary at
            0 and only a few people live a very long distance from the nearest hardware
            store.'
          feedback: Correct, awesome work
        ans2:
          value: 'Left skewed: The distribution of number of distance to the nearest
            hardware store is likely left skewed as there is a natural boundary at
            0 and only a few people live a very long distance from the nearest hardware
            store.'
        ...
        ans3:
          value: 'Symmetric: The distribution of number of distance to the nearest
            hardware store is likely symmetric as there is a natural boundary at 0
            and only a few people live a very long distance from the nearest hardware
            store.'
          feedback: Not quite, the reasoning here is mostly correct, think over what
